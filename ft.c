//flow meter using timer1 without prescalar and 5ms delay with issue of INT1 ACTIVATING FIRST RESOLVED IN ft2.c
...
ISR(INT0_vect)
{
	...
	EIMSK= 0b00000110;      //setting interrupt1 on and interrupt0 off
	EICRA=0X0C;              //setting interrupt1 for rising edge
//	sei();
//	while(1)
//	{
//		
...
//	ticks=(((float)overflow) * 0.009984) + (((float)tcntres) * 0.000128);  //for 1024 prescalar with calculation changes
	...
	ticks=(((float)overflow) * 0.005) + (((float)tcntres) * 0.000000125);  //for 1024 prescalar with calculation changes

	//sprintf(ticksres,"t=%.2f",ticks);
	//LCD_String_xy(0,0,ticksres);
	
	flow=((volume*60)/ticks);
	sprintf(flowres,"t=%.2f fl=%lf",ticks,flow);
